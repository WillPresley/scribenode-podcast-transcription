...
export default function App() {
  ...
  const [analysisErrors, setAnalysisErrors] = useState<Record<AnalysisMode, string | null>>({
    summary: null,
    key_takeaways: null,
    chapters: null,
    social_media: null,
  ...
  const generateAnalysis = async (mode: AnalysisMode) => {
    ...
    setAnalysisErrors(prev => ({ ...prev, [mode]: null }));
    ...
    try {
      ...
      if (!response.ok) {
        let msg = data?.error;
        if (!msg) {
          if (response.status === 504 || response.status === 502 || rawText.includes("504") || rawText.includes("Gateway") || rawText.includes("timeout")) {
            msg = "Insights generation timed out while waiting for AI models. Model failover has been accelerated—please retry.";
          } else if (rawText.includes("<!doctype") || rawText.includes("<html")) {
            msg = `Server returned an unexpected response (HTTP ${response.status}). Please try again.`;
          } else {
            msg = rawText || `Insights generation failed with HTTP ${response.status}.`;
          }
        }
        throw new Error(msg);
      }
      ...
      if (!data) {
        if (rawText.includes("<!doctype") || rawText.includes("<html")) {
          throw new Error("Received an unexpected HTML response instead of JSON. The server may have timed out. Please retry.");
        }
        try {
          data = JSON.parse(rawText);
        } catch {
          throw new Error("Unable to parse server response as JSON. Please try again.");
        }
      }

      if (!data.result) {
        throw new Error(data.error || "No content was returned by the AI model. Please retry.");
      }
      ...
      const friendlyMsg = err?.message || "Failed to generate insights.";
      setAnalysisErrors(prev => ({ ...prev, [mode]: friendlyMsg }));
    } finally {
                    ...
                    {(["summary", "key_takeaways", "chapters", "social_media"] as AnalysisMode[]).map((tab) => (
                      ...
                      >
                        ...
                        {job && (job[tab] || analysisResults[tab]) ? (
                          ...
                          <span className="w-1.5 h-1.5 rounded-full bg-amber-500 ml-0.5" title="Generation notice" />
                        ) : null}
                      </button>
                    ))}
                  </div>
                  ...
                  {analysisResults[activeAnalysisTab] && !loadingAnalysis[activeAnalysisTab] && (
                    <div className="bg-slate-50 border-b border-slate-200/80 px-3 sm:px-4 py-2 flex flex-wrap items-center justify-between gap-1.5 shrink-0 select-none">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        >
                          <RefreshCw className="h-2.5 w-2.5 text-slate-500" /> Regenerate
                        </button>
                        <button
                  ...
                  <div className="flex-1 overflow-y-auto p-4 sm:p-5 bg-white relative">
                    <AnimatePresence mode="wait">
                      ...
                      {analysisResults[activeAnalysisTab] ? (
                        ...
                        >
                          {analysisErrors[activeAnalysisTab] ? (
                            <div className="w-full max-w-md mx-auto p-4 rounded-xl bg-amber-50/90 border border-amber-200 text-left space-y-2.5 shadow-sm">
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex items-center gap-2">
                                  <AlertTriangle className="h-4 w-4 text-amber-600 shrink-0" />
                                  <p className="text-xs font-bold text-amber-900">Insight Generation Notice</p>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => setAnalysisErrors(prev => ({ ...prev, [activeAnalysisTab]: null }))}
                                  className="text-amber-500 hover:text-amber-700 p-0.5 rounded cursor-pointer"
                              ...
                              <div className="pt-1 flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => generateAnalysis(activeAnalysisTab)}
                                  className="bg-amber-600 hover:bg-amber-700 text-white font-bold text-[10px] uppercase tracking-wider py-1.5 px-3 rounded shadow-sm transition-colors flex items-center gap-1.5 cursor-pointer"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => setAnalysisErrors(prev => ({ ...prev, [activeAnalysisTab]: null }))}
                                  className="text-[10px] text-amber-700 hover:text-amber-900 font-medium px-2 py-1 cursor-pointer"
                                >
                                  Dismiss
                                </button>
                              </div>
